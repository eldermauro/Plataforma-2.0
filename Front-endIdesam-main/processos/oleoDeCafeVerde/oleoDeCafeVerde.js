...
function constroiCard(id, extrativista, local, quantidade) {
    ...
    const materia_prima = 'cafe verde';
...
async function listarLotes() {
    ...
    containerApi.forEach((element) => {
        ...
        let result = element.materia_prima.indexOf("cafe verde")
        element.materia_prima = element.materia_prima.slice(result, result + 10)
        if (element.materia_prima == /* materia prima em questão */ 'cafe verde') {
...
async function listarProcessos() {
    ...
    <h1 id="titulo-processo">Processos em andamento</h1>
    `
