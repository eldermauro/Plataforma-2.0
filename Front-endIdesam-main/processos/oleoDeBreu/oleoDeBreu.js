...
const destilacao = document.querySelector('.destilacao');
...
function mudaClasses(escolhe) {
    switch (escolhe) {
        case "btn-selecao-primaria":
            ...
            destilacao.classList.remove('inativo');
            destilacao.classList.add('ativo');
        ...
        case "btn-destilacao":
            destilacao.classList.remove('ativo');
            destilacao.classList.add('inativo');
...
async function listarLotes() {
    ...
    containerApi.forEach((element) => {
        ...
        let result = element.materia_prima.indexOf("breu")
        element.materia_prima = element.materia_prima.slice(result, result + 4)
        if (element.materia_prima == /* materia prima em questão */ 'breu') {
...
//capturando valores dos inputs selecaoPrimaria e criando o processo na tabela processos do banco de dados//
...
finalizarSelecaoPrimaria.addEventListener('click', () => {
    const selecaoPrimaria = document.querySelector('[data-selecaoPrimariaQuantidadeSelecionada]');
    ...
    const quantidade_de_entrada = selecaoPrimaria.value;
...
async function criaProcesso(lote_de_entrada, finalizado, quantidade_de_entrada) {
    ...
    preencheDestilacao(conexaoConvertida.id)
...
//preencher, finalizar Destilacao e atualizar tabela processos//

async function preencheDestilacao(id) {
    const conexao = await fetch(`http://localhost:3000/processos/${id}`)
    const conexaoConvertida = await conexao.json();

    let destilacaoEntrada = document.querySelector('[data-destilacaoEntrada]');
    destilacaoEntrada.value = conexaoConvertida.Quantidade_de_entrada;
    let destilacaoId = document.querySelector('[data-destilacaoId]');
    destilacaoId.value = id;
}
...
const finalizarDestilacao = document.querySelector("#btn-destilacao");

finalizarDestilacao.addEventListener('click', async () => {
    let id = document.querySelector('[data-destilacaoId]').value;
    let finalizado = false;
    let quantidade = document.querySelector('[data-destilacaoQuantidadeDestilada').value;
...
async function preencheFiltragem(id) {
    ...
    let filtragemId = document.querySelector('[data-filtragemId]');
...
async function configEntradaProcessosPendentes(id) {
    ...
    let destilacao = processoPendenteConvertido.Destilacao
    ...
    if (destilacao == true) {
        ...
        let destilacaoEntrada = document.querySelector('[data-destilacaoEntrada]');
        destilacaoEntrada.value = processoPendenteConvertido.Quantidade_de_entrada;
        const destilacaoId = document.querySelector('[data-destilacaoId]');
        destilacaoId.value = id;
    } else if (filtragem == true) {
        escolhe = 'btn-destilacao'
        let filtragemEntrada = document.querySelector('[data-filtragemEntrada]');
